...
func (g *GalaxyData) Report(argv []string, galaxyPath string, testMode, verboseMode bool) error {
	...
	for _, species := range speciesFilter {
		...
		if err := species.Report(report_file, galaxyPath, g.TurnNumber, testMode, ignore_field_distorters, truncate_name, DoLocations(g), g.GetPlanet, g.GetSpeciesByNumber, g.AllSpecies()); err != nil {
			return err
		}
